...
class EnhancedConsciousnessMonitor:
    def __init__(self, csv_file="mind_monitor_complete.csv", window_seconds=0.75, 
        ...
        self.macro_update = macro_update or (macro_window / 2)  # Default to half window size
        
        # Anxiety detection parameters
        ...
        # Macro analysis tracking
        self.macro_buffer = deque()  # Rolling buffer for macro window data
        self.last_macro_analysis = 0
        
        # Beta trend analysis for anxiety detection
        self.beta_history = deque(maxlen=self.beta_trend_window)  # Track last N readings for trend analysis
        self.trend_analysis_enabled = trend_analysis
        self.macro_state_history = deque()  # Track states detected in macro window
        self.macro_trends = {}  # Track trends for each band
        ...
        self.DETECTION_RULES = {
            "security_guard": {
                ...
                "conditions": {
                    ...
                    "alpha_exemption": 80,  # Don't trigger if alpha > 80% AND beta < 15% (true meditation)
                    "beta_exemption_max": 15,  # Only exempt if beta is also low
            ...
            "anxiety_escalation": {
                "priority": 6,
                "conditions": {"beta_trend": "increasing", "beta_min_change": 5, "alpha_min": 40},
                "emoji": "📈",
                "insights": ["📈 Anxiety building - beta wave escalation detected", "⚠️ Cognitive worry increasing while maintaining some regulation"]
            },
            "subtle_anxiety": {
                "priority": 7,
                "conditions": {"beta_min": 20, "alpha_min": 50, "alpha_max": 85, "delta_max": 25},
                "emoji": "⚠️",
                "insights": ["⚠️ Cognitive anxiety with somatic regulation", "🧠 Worried thoughts with regulated nervous system"]
            },
            "integration": {
                "priority": 8,
                "conditions": {
                    "delta_min": 25, "delta_max": 45,
                    "alpha_min": 30, "alpha_max": 50,
                    "beta_min": 15, "beta_max": 35,
                    "gamma_db_change_min": 1.5  # Detects gamma spikes
                },
                "emoji": "🧩",
                "insights": ["🧩 Integration state - processing and filing memories", "💭 Mixed delta/alpha suggests emotional integration", "🗃️ Brain organizing recent experiences into long-term storage"]
            },
            "cautious_part": {
                "priority": 9,
                "conditions": {"alpha_min": 50, "alpha_max": 70, "delta_min": 15, "beta_min": 15},
                "emoji": "🛡️",
                "insights": ["🛡️ Cautious part assessing - protective awareness", "👁️ Scanning environment for potential threats or concerns"]
            },
            "recovery": {
                "priority": 10,
    ...
    def interpret_enhanced_state(self, eeg_analysis, optics_analysis=None):
        ...
        try:
            ...
            # Track beta trend for anxiety detection
            current_beta = percentage_ratios.get('beta', 0)
            self.beta_history.append(current_beta)
            
            # Calculate beta trend if we have enough history
            beta_trend = None
            beta_trend_strength = 0
            if len(self.beta_history) >= 3:
                beta_trend_strength = self.beta_history[-1] - self.beta_history[-3]  # Change over last 3 readings
                if beta_trend_strength > 5:  # At least 5% increase
                    beta_trend = "increasing"
                elif beta_trend_strength < -5:
                    beta_trend = "decreasing"
                else:
                    beta_trend = "stable"
            
            # Add trend info to db_changes for rule evaluation
            db_changes_dict['beta_trend'] = beta_trend
            db_changes_dict['beta_trend_strength'] = beta_trend_strength
            ...
            if self.debug:
                ...
                if beta_trend:
                    print(f"Debug - Beta trend: {beta_trend} ({beta_trend_strength:+.1f}% over 3 readings)")
            ...
            try:
                ...
                # Enhanced cautious part context for anxiety states
                if "CAUTIOUS PART" in state:
                    beta_percent = percentage_ratios.get('beta', 0)
                    if beta_percent > 20:
                        enhanced_insights.append("⚠️ Cautious part activated during elevated cognitive anxiety")
                        enhanced_insights.append("🛡️ Protective system monitoring for threats while anxious")
                    elif beta_trend == "increasing":
                        enhanced_insights.append("📈 Cautious part responding to building anxiety")
    ...
    def _test_rule_conditions(self, rule_name, rule, bands, db_changes):
        ...
        conditions = rule['conditions'].copy()  # Make a copy to avoid modifying original
        
        # Apply anxiety sensitivity scaling to anxiety-related rules
        anxiety_rules = ['security_guard', 'subtle_anxiety', 'anxiety_escalation', 'cautious_part']
        if rule_name in anxiety_rules and self.anxiety_sensitivity != 1.0:
            # Scale beta thresholds for anxiety detection
            if 'beta_min' in conditions:
                conditions['beta_min'] = conditions['beta_min'] / self.anxiety_sensitivity
            if 'beta_min_change' in conditions:
                conditions['beta_min_change'] = conditions['beta_min_change'] / self.anxiety_sensitivity
        ...
        for condition, value in conditions.items():
            if condition.endswith('_min') and not condition.endswith('_db_change_min'):
                ...
                if db_changes.get(db_change_key, 0) < value:
                    return False
            elif condition == 'beta_trend':
                # Handle beta trend conditions
                if db_changes.get('beta_trend') != value:
                    return False
            elif condition == 'beta_min_change':
                # Handle minimum beta change for trend detection
                if abs(db_changes.get('beta_trend_strength', 0)) < value:
                    return False
            elif condition == 'alpha_maintained':
                # Special condition: alpha stays relatively stable during beta changes
                # This could be expanded based on alpha variance tracking
                # For now, just check if alpha is > 40%
                if bands.get('alpha', 0) < 40:
                    return False
    ...
    def _test_security_guard_conditions(self, conditions, bands, db_changes):
        ...
        # Meditation exemption - only exempt if BOTH high alpha AND low beta (true meditation)
        # Don't exempt if beta is elevated (anxious thoughts with regulated body)
        alpha_threshold = conditions.get('alpha_exemption', 80)
        beta_threshold = conditions.get('beta_exemption_max', 15)
        
        if not self.disable_meditation_exemption and alpha_percent > alpha_threshold and beta_percent < beta_threshold:
            if self.debug:
                print(f"Debug - Security guard exempted due to meditation state (alpha={alpha_percent:.1f}%, beta={beta_percent:.1f}%)")
            return False
    ...
    def monitor_realtime(self):
        ...
        if self.dual_mode:
            print(f"🔄 Starting enhanced dual-mode monitoring (micro + macro {self.macro_window}s)...")
        elif self.macro_only:
            print(f"🔄 Starting enhanced macro-only monitoring ({self.macro_window}s windows)...")
        else:
            ...
            while True:
                ...
                if data is not None and (not hasattr(data, 'empty') or not data.empty):
                    if self.data_format == "muse_player" and len(data.get('timestamp', [])) > 10:
                        ...
                        # Only display micro results if not in macro-only mode
                        if not self.macro_only:
                            self.display_enhanced_results(interpretation, latest_timestamp)
                        
                        # Update macro buffer if using macro analysis
                        if self.dual_mode or self.macro_only:
                            current_time = time.time()
                            state = interpretation.get('state', 'UNKNOWN')
                            ratios = interpretation.get('ratios', {})
                            fnirs = interpretation.get('fnirs', 0)
                            
                            self._update_macro_buffer(current_time, ratios, state, fnirs)
                            
                            # Check if it's time for macro analysis
                            if (current_time - self.last_macro_analysis >= self.macro_update):
                                patterns = self._detect_macro_patterns()
                                if patterns:
                                    trends = self._analyze_macro_trends() if self.trend_analysis else {}
                                    
                                    # Display macro analysis
                                    macro_output = self._format_macro_output(patterns, trends, int(self.macro_window))
                                    print(f"\n{macro_output}")
                                    
                                    # Show detailed analysis if requested
                                    if self.trend_analysis:
                                        details = self._format_macro_details(patterns, trends)
                                        for detail in details:
                                            print(detail)
                                    
                                    if self.macro_only:
                                        print(f"         Next analysis in {self.macro_update:.0f}s...")
                                    
                                    self.last_macro_analysis = current_time
    ...
    def _update_macro_buffer(self, timestamp, ratios, state, fnirs):
        """Add data point to macro analysis buffer"""
        data_point = {
            'timestamp': timestamp,
            'ratios': ratios.copy(),
            'state': state, 
        ...
        self.macro_buffer.append(data_point)
        
        # Keep only data within macro window
        current_time = time.time()
        while (self.macro_buffer and 
               current_time - self.macro_buffer[0]['timestamp'] > self.macro_window):
            self.macro_buffer.popleft()
    
    def _analyze_macro_trends(self):
        """Analyze trends over the macro window"""
        if len(self.macro_buffer) < 10:  # Need minimum data points
            return {}
        
        trends = {}
        timestamps = [point['timestamp'] for point in self.macro_buffer]
        relative_times = [t - timestamps[0] for t in timestamps]
        
        for band in ['delta', 'theta', 'alpha', 'beta', 'gamma']:
            values = [point['ratios'].get(band, 0) for point in self.macro_buffer]
            
            if len(values) >= 10:
                try:
                    slope, intercept, r_value, p_value, std_err = linregress(relative_times, values)
                    ...
                    # Calculate total change over window
                    ...
                    trends[band] = {
                        ...
                        'confidence': abs(r_value)  # How linear the trend is
                    }
                except:
                    trends[band] = {
                        'direction': 'unknown',
    ...
    def _detect_macro_patterns(self):
        """Detect sustained patterns over macro window"""
        if len(self.macro_buffer) < 10:
            return None
        
        # Calculate average ratios over the macro window
        avg_ratios = {}
        for band in ['delta', 'theta', 'alpha', 'beta', 'gamma']:
            values = [point['ratios'].get(band, 0) for point in self.macro_buffer]
            avg_ratios[band] = sum(values) / len(values)
        
        # Apply existing detection rules to macro-averaged data
        fake_interpretation = {'ratios': avg_ratios}
        macro_state = self._detect_consciousness_state(fake_interpretation)
        
        # Count state occurrences for confidence
        state_counts = {}
        for point in self.macro_buffer:
            state = point['state']
            state_counts[state] = state_counts.get(state, 0) + 1
        
        # Find primary state (most common)
        if state_counts:
            primary_state = max(state_counts.keys(), key=lambda k: state_counts[k])
            confidence = (state_counts[primary_state] / len(self.macro_buffer)) * 100
        else:
            primary_state = "UNKNOWN"
            confidence = 0
        
        # Detect state transitions
        states_detected = list(state_counts.keys())
        ...
        return {
            ...
            'state_durations': state_counts,
            'avg_ratios': avg_ratios
        }
    
    def _format_macro_output(self, patterns, trends, duration_s):
        """Format macro analysis output"""
        if not patterns:
            return f"[MACRO {duration_s}s] Insufficient data for analysis"
        
        # Determine trend arrow for primary pattern
        primary_state = patterns['primary_state']
        macro_state = patterns['macro_state']
        confidence = patterns['confidence']
        
        # Find dominant trend
        dominant_band = max(patterns['avg_ratios'].keys(), 
                          key=lambda k: patterns['avg_ratios'][k])
        trend_arrow = trends.get(dominant_band, {}).get('arrow', '➡️')
        
        # Detect progression patterns
        if primary_state in ["RELAXED", "JHANA", "FLOW_STATE"] and trend_arrow == "↗️":
            progression = "MEDITATION DEEPENING"
        elif primary_state in ["YOUNG_PART", "VULNERABLE"] and trend_arrow == "➡️":
            progression = "EMOTIONAL PROCESSING"
        elif primary_state in ["FOCUSED", "PEAK_FOCUS"] and trend_arrow == "↗️":
            progression = "CONCENTRATION BUILDING"
        else:
            progression = primary_state
        
        # Format main line
        output = f"[MACRO {duration_s}s] {progression} {trend_arrow} | "
        output += f"Primary: {primary_state} | Confidence: {confidence:.0f}%"
        
        return output
    
    def _format_macro_details(self, patterns, trends):
        """Format detailed macro analysis"""
        if not patterns or not trends:
            return []
        ...
        # Trend analysis
        significant_trends = []
        for band, trend in trends.items():
            if abs(trend['total_change']) > 5:  # Significant change
                change_str = f"{trend['total_change']:+.1f}%"
                significant_trends.append(f"{band.capitalize()} {trend['direction']} ({change_str})")
        
        if significant_trends:
            details.append("         Trend: " + ", ".join(significant_trends))
        
        # Pattern description
        avg_ratios = patterns['avg_ratios']
        if avg_ratios['alpha'] > 50:
            if avg_ratios['theta'] > 20:
                pattern_desc = "Deep regulation with emotional processing"
            else:
                pattern_desc = "Sustained regulation state"
        elif avg_ratios['beta'] > 40:
            pattern_desc = "High arousal with elevated activity"
        elif avg_ratios['delta'] > 40:
            pattern_desc = "Low arousal, restful state"
        else:
            pattern_desc = "Balanced consciousness state"
        
        details.append(f"         Pattern: {pattern_desc}")
        
        # States summary
        states_detected = patterns['states_detected']
        state_durations = patterns['state_durations']
        total_points = sum(state_durations.values())
        
        state_summary = []
        for state in states_detected[:3]:  # Top 3 states
            duration_pct = (state_durations[state] / total_points) * 100
            if duration_pct > 10:  # Only show significant states
                state_summary.append(f"{state}({duration_pct:.0f}%)")
        
        if state_summary:
            details.append("         States detected: " + ", ".join(state_summary))
...
def main():
    ...
    parser.add_argument("--macro-window", type=float, default=60.0, help="Macro analysis window in seconds for pattern detection (default: 60)")
    parser.add_argument("--dual-mode", action="store_true", help="Run both micro (real-time) and macro (pattern) analysis simultaneously")
    parser.add_argument("--macro-only", action="store_true", help="Only show macro patterns, suppress micro readings")
    parser.add_argument("--trend-analysis", action="store_true", help="Include trend detection (increasing/decreasing/stable)")
    parser.add_argument("--macro-update", type=float, help="Macro analysis update interval in seconds (default: half of macro window)")
    parser.add_argument("--anxiety-sensitivity", type=float, default=1.0, help="Multiplier for anxiety detection sensitivity (0.5=less sensitive, 2.0=more sensitive)")
    parser.add_argument("--disable-meditation-exemption", action="store_true", help="Disable meditation exemption for security guard detection during calibration")
    parser.add_argument("--beta-trend-window", type=int, default=5, help="Number of readings to analyze for beta trend detection (default: 5)")
