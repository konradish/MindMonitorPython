...
```javascript
stack(note("<C3 G3 Eb3 Bb2>/4").s("gm_pad_choir").room(0.8).gain(0.4), s("~ hh*4").gain(0.2).lpf(800), note("c2 g1").s("gm_acoustic_bass").slow(2).gain(0.5))
```
...
```javascript
stack(s("bd [~ bd] ~ bd, ~ sd ~ [sd sd]").bank("RolandTR808"), note("<[c4 e4] [g4 b4] [a4 f4]>*2").s("gm_marimba").delay(0.3).room(0.5), s("arpy*8").n(irand(12)).gain(0.3))
```
...
```javascript
stack(note("<Dm7 Am7 Fmaj7 Em7>/8").voicing().s("gm_pad_warm").room(0.9), s("~ rim").slow(4).gain(0.15).room(0.8), note("d1").s("sine").decay(4).sustain(0.1).lpf(200)).slow(2)
```
...
```javascript
stack(s("bd*4, ~ sd ~ sd, hh*8").bank("RolandTR909").gain(0.5), note("<c3 g2 c3 g2>/2").s("gm_synth_bass_1").lpf(400)).lpf(sine.range(800,2000).slow(16))
```
...
```javascript
stack(note("<Cm9 Abmaj9 Ebmaj7 Bb9>/4").voicing().s("gm_pad_choir").room(0.95), note("c4 g4 eb5 g5").s("gm_strings").arp("up").slow(2).delay(0.4), s("bd ~ ~ bd, ~ sd ~ sd").slow(2).gain(0.4)).shape(0.2)
```
